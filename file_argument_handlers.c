...
/**
 * read_commands_file - Reads commands from the argument File
...
void read_commands_file(char *file, char **argv)
{
	...
	if (fp == NULL)
	{
		display_file_error(argv, count);
		exit(127);
	}
	while ((getline(&line, &len, fp)) != -1)
	{
		count++;
		process_command_file(line, count, fp, argv);
	}
	if (line)
		free(line);
	fclose(fp);
...
/**
 * process_command_file - Parse commands and handle their execution
 * @line: Line from file
 * @count: Error counter
 * @fp: File descriptor
...
void process_command_file(char *line, int count, FILE *fp, char **argv)
{
	...
	{
		stat = execute_command(cmd, line, count, argv);
		free(cmd);
	}
}
